...
from mobility_advisor.api.recommendation import builder, finalize
from mobility_advisor.models import Recommendation
from mobility_advisor.store import decisions
...
def test_no_life_events_no_deferral(maja):
    ...
    result = decisions.detect_pending_portfolio_decision()
...
def test_hold_row_injected_and_promoted_for_stefan(tmp_path, monkeypatch):
    ...
    # deterministic post-processing in api/recommendation/: builder.build_alternatives_
    # from_optimization() injects a "Hold pending decision" row whenever
    # decisions.detect_pending_portfolio_decision() fires, and
    # finalize.enforce_hold_when_decision_pending() promotes it over whatever
    ...
    rec = finalize.enforce_hold_when_decision_pending(rec)
...
def test_deferral_closes_once_events_are_in_the_past(stefan, monkeypatch):
    ...
    monkeypatch.setattr(clock, "MOCK_TODAY", date(2026, 10, 1))
    assert decisions.detect_pending_portfolio_decision()["exists"] is False
...
def test_deferral_not_triggered_when_decision_is_far_off(stefan, monkeypatch):
    ...
    monkeypatch.setattr(clock, "MOCK_TODAY", date(2025, 1, 1))
    assert decisions.detect_pending_portfolio_decision()["exists"] is False
