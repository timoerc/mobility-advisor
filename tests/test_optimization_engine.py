...
unset (see engine/geo.py's _cached_geocode/_offline_geocode and integrations/ors.py)."""
...
from mobility_advisor import clock, paths
from mobility_advisor.engine import aggregates, calibration, factors, geo, optimizer, pricing, projection, simulation
...
def test_sparpreis_trip_bc25_and_bc50_discount_equally():
    price = 100.0
    bc25_price = pricing.apply_subscription_discount("rail_intercity", price, 400, [_BC25], fare_class="spar")
    bc50_price = pricing.apply_subscription_discount("rail_intercity", price, 400, [_BC50], fare_class="spar")
    assert bc25_price == bc50_price == 75.0


def test_flexpreis_trip_bc50_discounts_more_than_bc25():
    price = 100.0
    bc25_price = pricing.apply_subscription_discount("rail_intercity", price, 400, [_BC25], fare_class="flex")
    bc50_price = pricing.apply_subscription_discount("rail_intercity", price, 400, [_BC50], fare_class="flex")
...
def test_fare_class_defaults_to_spar_when_unspecified():
    price = 100.0
    default_price = pricing.apply_subscription_discount("rail_intercity", price, 400, [_BC50])
    spar_price = pricing.apply_subscription_discount("rail_intercity", price, 400, [_BC50], fare_class="spar")
...
def test_car_share_no_subscription_uses_miles_basis_walkup_floor():
    ...
    price = pricing.apply_subscription_discount("car_share", bare_synthetic_price, 10.0, [])
    expected_floor = round(
        pricing._MILES_BASIS_BASE_KM_RATE_EUR * 10.0
        + pricing._MILES_BASIS_UNLOCK_FEE_EUR
        + pricing._MILES_BASIS_PROTECTION_FEE_EUR,
        2,
    )
...
def test_car_share_paid_tier_still_undercuts_the_walkup_floor():
    ...
    price = pricing.apply_subscription_discount("car_share", 5.0, 10.0, [miles_gold])
...
def test_non_car_share_mode_still_uses_estimated_price_eur_as_floor():
    ...
    price = pricing.apply_subscription_discount("rail_intercity", 42.5, 400, [])
    assert price == 42.5
...
def test_dominant_fare_class_majority_flex():
    ...
    assert calibration._dominant_fare_class(ticket_types) == "flex"


def test_dominant_fare_class_majority_spar():
    ticket_types = ["Sparpreis, 2. Klasse"] * 6 + ["Flexpreis, 2. Klasse"] * 4
    assert calibration._dominant_fare_class(ticket_types) == "spar"
...
def test_dominant_fare_class_tie_defaults_to_spar():
    ...
    assert calibration._dominant_fare_class(ticket_types) == "spar"
    assert calibration._dominant_fare_class([None, None]) == "spar"
...
def test_compute_portfolio_score_lower_cost_wins_under_cost_weight():
    ...
    scored = simulation.compute_portfolio_score(
...
def test_compute_portfolio_score_independent_of_irrelevant_alternatives():
    ...
    without_decoy = simulation.compute_portfolio_score([low_co2, baseline], weights)
    with_decoy = simulation.compute_portfolio_score([low_co2, baseline, decoy], weights)
...
def test_compute_portfolio_score_no_valid_results_is_error():
    result = simulation.compute_portfolio_score([{"status": "error"}], {})
...
def test_generalized_cost_rates_scales_with_weights():
    vot_time_heavy, co2_time_heavy = simulation._generalized_cost_rates(
        {"cost_weight": 0.3, "time_weight": 0.5, "sustainability_weight": 0.2}
    )
    vot_cost_heavy, co2_cost_heavy = simulation._generalized_cost_rates(
...
def test_generalized_cost_rates_floors_zero_cost_weight():
    ...
    zero = simulation._generalized_cost_rates(
        {"cost_weight": 0.0, "time_weight": 0.5, "sustainability_weight": 0.5}
    )
    floored = simulation._generalized_cost_rates(
...
def test_generalized_cost_rates_none_weights_is_pure_cost():
    assert simulation._generalized_cost_rates(None) == (0.0, 0.0)
...
def test_mode_shares_sums_to_one():
    shares = simulation._mode_shares([_RAIL_REGIONAL_ALT, _CAR_ALT], [], "spar", 100, _DEFAULT_WEIGHTS)
    assert sum(share for _, _, share in shares) == pytest.approx(1.0)


def test_mode_shares_single_alternative_gets_full_share():
    shares = simulation._mode_shares([_RAIL_REGIONAL_ALT], [], "spar", 100, _DEFAULT_WEIGHTS)
...
def test_mode_shares_rail_discount_raises_rail_share_on_rail_eligible_trip():
    no_sub = {
        alt["mode"]: share
        for alt, _, share in simulation._mode_shares(
...
def test_simulate_portfolio_time_and_co2_differ_across_portfolios(isolated_main_data_dir):
    ...
    empty = simulation.simulate_portfolio([], weights=_DEFAULT_WEIGHTS)
    with_dticket = simulation.simulate_portfolio(["db_deutschlandticket"], weights=_DEFAULT_WEIGHTS)
...
def test_no_travel_reduction_signal_is_a_no_op(isolated_data_dir):
    ...
    factor, warnings = calibration._travel_reduction_factor()
    assert factor == 1.0
    assert warnings == []


def test_travel_reduction_signal_prorates_by_remaining_days(isolated_data_dir):
    event_date = clock.MOCK_TODAY + timedelta(days=78)
    _write_life_events(isolated_data_dir, [{
        "category": "other",
        ...
        "source_mail_id": None,
        "detected_on": clock.MOCK_TODAY.isoformat(),
    }])
    factor, warnings = calibration._travel_reduction_factor()
...
def test_travel_reduction_signal_outside_12_months_is_ignored(isolated_data_dir):
    event_date = clock.MOCK_TODAY + timedelta(days=400)
    _write_life_events(isolated_data_dir, [{
        "category": "other",
...
def test_non_travel_reduction_signal_does_not_damp(isolated_data_dir):
    ...
    _write_life_events(isolated_data_dir, [{
        ...
        "event_date": (clock.MOCK_TODAY + timedelta(days=30)).isoformat(),
...
def test_offline_geocode_known_city_returns_lng_lat(monkeypatch):
    monkeypatch.setattr(geo, "_city_coords_cache", None)
    lng, lat = geo._offline_geocode("Berlin")
...
def test_offline_geocode_normalizes_raw_station_strings(monkeypatch):
    monkeypatch.setattr(geo, "_city_coords_cache", None)
    result = geo._offline_geocode("Frankfurt (Main) Hbf")
...
def test_offline_geocode_unknown_place_returns_none(monkeypatch):
    monkeypatch.setattr(geo, "_city_coords_cache", None)
    assert geo._offline_geocode("Nonexistent Fantasy City") is None


def test_cached_geocode_uses_offline_fallback_when_ors_key_absent(monkeypatch):
    monkeypatch.setattr(geo, "ORS_API_KEY", "")
    ...
    monkeypatch.setattr(geo, "geocode", _fail_if_called)
    result = geo._cached_geocode("Hamburg")
    assert result is not None
...
def test_merge_preserves_flex_fare_class_when_calendar_overrides_history(isolated_main_data_dir):
    ...
    result = projection.merge_projected_trip_sets()
...
# ── paths.clear_scratch_files: no stale cross-run/cross-persona results ────


def test_clear_optimization_scratch_files_removes_everything(tmp_path, monkeypatch):
    monkeypatch.setattr(paths, "DATA_DIR", tmp_path)
    for fname in paths.SCRATCH_FILES:
...
def test_clear_optimization_scratch_files_is_a_noop_when_absent(tmp_path, monkeypatch):
    monkeypatch.setattr(paths, "DATA_DIR", tmp_path)
    paths.clear_scratch_files()  # must not raise when nothing exists yet
...
def test_car_share_credit_never_exceeds_annual_budget_under_heavy_usage(isolated_main_data_dir):
    ...
    sim = simulation.simulate_portfolio(["miles_gold"], weights=None)
...
def test_local_aggregate_trip_created_for_subthreshold_regional_routes():
    ...
    trip, warning = aggregates._build_local_aggregate_trip(local_routes, reduction_factor=1.0)
...
def test_local_aggregate_trip_none_when_damped_to_zero():
    ...
    trip, warning = aggregates._build_local_aggregate_trip(local_routes, reduction_factor=0.0)
    assert trip is None
    assert warning == ""
...
def test_commute_aggregate_trip_from_office_days(isolated_data_dir):
    ...
    trip, warning = aggregates._build_commute_aggregate_trip(reduction_factor=1.0)
    assert trip is not None
    assert trip["frequency_per_year"] == 4 * aggregates._WORKING_WEEKS_PER_YEAR * 2
    assert trip["origin"] == "various"
...
def test_commute_aggregate_trip_none_when_fully_remote(isolated_data_dir):
    ...
    trip, warning = aggregates._build_commute_aggregate_trip(1.0)
    assert trip is None
    assert warning == ""
...
def test_intra_city_aggregate_trip_created_from_car_share_trips(isolated_data_dir):
    ...
    trip, warning = aggregates._build_intra_city_aggregate_trip(car_trips, data_window_months=11)
    assert trip is not None
    assert trip["origin"] == "various"
...
def test_intra_city_aggregate_trip_excludes_non_car_modes(isolated_data_dir):
    ...
    trip, warning = aggregates._build_intra_city_aggregate_trip(mixed_trips, data_window_months=12)
    assert trip is not None
...
def test_intra_city_aggregate_trip_none_when_no_car_trips(isolated_data_dir):
    ...
    trip, warning = aggregates._build_intra_city_aggregate_trip(non_car_trips, data_window_months=12)
    assert trip is None
    assert warning == ""
...
def test_derive_projected_trips_from_history_folds_same_city_trips_into_intra_city_aggregate(
    ...
    monkeypatch.setattr(geo, "ORS_API_KEY", "")
    monkeypatch.setattr(geo, "driving_route", lambda *a, **kw: None)
    history = {"trips": [
        {
    ...
    result = projection.derive_projected_trips_from_history()
    assert result["status"] == "ok"
...
def test_damped_to_zero_long_distance_route_is_dropped(isolated_data_dir, monkeypatch):
    ...
    event_date = clock.MOCK_TODAY + timedelta(days=5)
    _write_life_events(isolated_data_dir, [{
...
def test_rail_fare_calibration_excludes_flat_rate_and_zero_cost_trips(isolated_data_dir):
    ...
    ratio, max_dist, warnings = calibration._rail_fare_calibration_ratio()

    expected_synthetic = 2 * factors.estimate_trip_price("rail_intercity", 424)
...
def test_optimize_all_categories_produces_a_coherent_ranking(persona, tmp_path, monkeypatch):
    ...
    projection.derive_projected_trips_from_history()
    projection.derive_car_usage_trips()
    projection.merge_projected_trip_sets()
    result = optimizer.optimize_all_categories()
...
def test_current_portfolio_with_skip_id_has_no_duplicate_entry(isolated_main_data_dir):
    ...
    # nothing to add or remove (builder.py rendered it as "No change.").
    trip = {
...
def test_compute_route_alternatives_never_offers_both_flight_modes(monkeypatch):
    ...
    domestic = geo.compute_route_alternatives("Berlin", "Stuttgart")
    ...
    # other endpoint.
    international = geo.compute_route_alternatives("Athens", "Zurich")
...
def test_local_tariff_trip_ignores_bahncard_but_deutschlandticket_still_covers_it():
    ...
    price_with_bahncard = pricing.apply_subscription_discount(
    ...
    price_with_deutschlandticket = pricing.apply_subscription_discount(
    ...
    price_national = pricing.apply_subscription_discount(
