"""Verifies the X-Language header middleware (api/app.py's _language_middleware) actually makes
...
from mobility_advisor.api.app import app
from mobility_advisor.api.routes import data as data_route
from mobility_advisor.i18n import get_language


def test_x_language_header_sets_request_language(monkeypatch):
    captured: list[str] = []
    original_catalog_lookup = data_route.catalog_lookup
    ...
    monkeypatch.setattr(data_route, "catalog_lookup", spy_catalog_lookup)
