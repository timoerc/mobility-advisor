...
def _read_catalog_bytes():
    static_dir = Path(__file__).parent.parent / "mobility_advisor" / "static"
    return (static_dir / "mobility_catalog.json").read_bytes()


def test_remove_existing_subscription(isolated_data_dir):
    result = mutations.apply_subscription_change(action="remove", target_subscription="Enterprise Silver")
...
def test_remove_nonexistent_target_is_error_no_write(isolated_data_dir):
    ...
    result = mutations.apply_subscription_change(action="remove", target_subscription="Nonexistent")
...
def test_ambiguous_new_product_is_error_no_write(isolated_data_dir):
    before = (isolated_data_dir / "current_subscriptions.json").read_bytes()
    result = mutations.apply_subscription_change(action="add", new_product="MILES")
    assert result["status"] == "error"
...
def test_add_nonexistent_product_is_error_no_write(isolated_data_dir):
    before = (isolated_data_dir / "current_subscriptions.json").read_bytes()
    result = mutations.apply_subscription_change(action="add", new_product="Nonexistent Product")
    assert result["status"] == "error"
    assert "no match" in result["error"]
    after = (isolated_data_dir / "current_subscriptions.json").read_bytes()
    assert before == after
...
def test_missing_required_param_is_error_no_backup(isolated_data_dir):
    result = mutations.apply_subscription_change(action="remove")
    assert result["status"] == "error"
...
def test_catalog_file_untouched_after_every_operation(isolated_data_dir, action, kwargs):
    ...
    result = mutations.apply_subscription_change(action=action, **kwargs)
    assert result["status"] == "applied"
...
def test_no_stray_temp_file_after_success_or_error(isolated_data_dir):
    mutations.apply_subscription_change(action="remove", target_subscription="Enterprise Silver")
    mutations.apply_subscription_change(action="remove", target_subscription="Nonexistent")
...
def test_token_fallback_no_false_positive_between_bahncard_tiers(isolated_data_dir):
    ...
    result = mutations.apply_subscription_change(action="add", new_product="BahnCard 25 (2nd class)")
    assert result["status"] == "error"
    assert "ambiguous" in result["error"]
