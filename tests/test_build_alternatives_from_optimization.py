"""Tests for api/recommendation/builder.py's build_alternatives_from_optimization() and
the deterministic warnings pipeline it feeds into Recommendation.dataQualityWarnings —
see CLAUDE.md's "Four-stage pipelines" section and the has_keep fix in
build_alternatives_from_optimization.
"""
...
from mobility_advisor.api.recommendation import builder
from mobility_advisor.engine import geo, optimizer, projection
from mobility_advisor.models import Recommendation
from mobility_advisor.store.scenarios import activate_scenario
...
def test_current_also_recommended_produces_two_distinct_no_action_rows(tmp_path, monkeypatch):
    ...
    # build_alternatives_from_optimization() also calls detect_pending_portfolio_decision(),
    # which reads paths.DATA_DIR/life_events.json — tmp_path has no life_events.json, so
    # load_life_events() correctly defaults to no events/no gate.
    ...
    alts = builder.build_alternatives_from_optimization()
...
def test_optimize_all_categories_persists_merged_warnings(tmp_path, monkeypatch):
    ...
    # builder.load_optimization_warnings() reads to populate
    ...
    assert builder.load_optimization_warnings() == result["warnings"]


def test_activate_from_scenario_clears_stale_scratch_files(tmp_path, monkeypatch):
    # B2 regression: switching personas via activate_scenario() (used by both
    # /api/activate and /api/profile) must not leave a PREVIOUS persona's derived
    # trip-projection/optimization files sitting in data/ —
    # build_alternatives_from_optimization() has no freshness check and would otherwise
