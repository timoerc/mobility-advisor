...
_DELTA_LINK_FILE = paths.PACKAGE_ROOT / "mail_delta_link.txt"
...
def save_raw(trips: list[dict]) -> None:
    raw_output = paths.DATA_DIR / "travel_history_raw.json"
    ...
    raw_output.write_text(json.dumps(result, indent=2, ensure_ascii=False), encoding="utf-8")
    print(f"travel_history_raw.json updated — {len(result['trips'])} trips total.")


def run(use_mock: bool = False) -> None:
    if use_mock:
        raw = json.loads((paths.DATA_DIR / "mail_raw.json").read_text(encoding="utf-8"))
